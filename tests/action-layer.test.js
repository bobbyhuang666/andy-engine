...
describe('Backward compatibility: agent/action/ re-exports', () => {
  it('agent/action/ActionCandidate re-exports createCandidate', () => {
    const old = require('../agent/action/ActionCandidate');
    expect(old.createCandidate).toBeDefined();
    expect(old.ACTION_TYPES).toBeDefined();
    expect(old.CANDIDATE_SOURCES).toBeDefined();
    expect(old.makeCandidateId).toBeDefined();
    expect(old.ActionCandidate).toBeDefined();
  });

  it('createCandidate produces valid candidate', () => {
    const { createCandidate } = require('../agent/action/ActionCandidate');
    const c = createCandidate({ type: 'rest', source: 'need', target: 'energy' });
    expect(c.type).toBe('rest');
    expect(c.source).toBe('need');
    expect(c.target).toBe('energy');
    expect(c.id).toBeDefined();
  });

  it('agent/action/UtilityScorer re-exports scoreCandidate/scoreCandidates', () => {
    const old = require('../agent/action/UtilityScorer');
    expect(old.scoreCandidate).toBeDefined();
    expect(old.scoreCandidates).toBeDefined();
    expect(typeof old.scoreCandidate).toBe('function');
  });

  it('agent/action/UtilitySelector re-exports selectAction', () => {
    const old = require('../agent/action/UtilitySelector');
    expect(old.selectAction).toBeDefined();
    expect(typeof old.selectAction).toBe('function');
  });

  it('agent/action/providers/ re-exports all providers', () => {
    const { CandidateProviderManager } = require('../agent/action/providers/CandidateProviderManager');
    expect(CandidateProviderManager).toBeDefined();

    const { CandidateProvider } = require('../agent/action/providers/CandidateProvider');
    expect(CandidateProvider).toBeDefined();

    const { ContinueCandidateProvider } = require('../agent/action/providers/ContinueCandidateProvider');
    expect(ContinueCandidateProvider).toBeDefined();

    const { NeedCandidateProvider } = require('../agent/action/providers/NeedCandidateProvider');
    expect(NeedCandidateProvider).toBeDefined();

    const { ScheduleCandidateProvider } = require('../agent/action/providers/ScheduleCandidateProvider');
    expect(ScheduleCandidateProvider).toBeDefined();

    const { BehaviorFieldCandidateProvider } = require('../agent/action/providers/BehaviorFieldCandidateProvider');
    expect(BehaviorFieldCandidateProvider).toBeDefined();

    const { ExploreCandidateProvider } = require('../agent/action/providers/ExploreCandidateProvider');
    expect(ExploreCandidateProvider).toBeDefined();

    const { SocializeCandidateProvider } = require('../agent/action/providers/SocializeCandidateProvider');
    expect(SocializeCandidateProvider).toBeDefined();
  });

  it('old createCandidate and new ActionCandidate produce same ID', () => {
    const { createCandidate } = require('../agent/action/ActionCandidate');
    const { ActionCandidate } = require('../src/action/ActionCandidate');

    const old = createCandidate({ type: 'rest', source: 'need', target: 'energy' });
    const neu = new ActionCandidate({ type: 'rest', source: 'need', target: 'energy' });

    expect(old.id).toBe(neu.id);
  });

  it('old providers produce candidates accepted by new scorer', () => {
    const { CandidateProviderManager } = require('../agent/action/providers/CandidateProviderManager');
    const { scoreCandidates } = require('../src/action/UtilityScorer');
    ...
    for (const s of scored) {
      ...
      expect(isNaN(s.score.total)).toBe(false);
    }
  });

  it('old selectAction produces ReasonTrace with expected fields', () => {
    const { selectAction } = require('../agent/action/UtilitySelector');
    const { ActionCandidate } = require('../src/action/ActionCandidate');

    const c = new ActionCandidate({ type: 'rest', source: 'need' });
    const scored = [{
      candidate: c,
      score: { total: 0.5, need: 0.5, emotion: 0, behavior: 0, memory: 0, relationship: 0, habit: 0, goal: 0, location: 0, world: 0, time: 0, constraint: 0, tendency: 0 },
    }];
