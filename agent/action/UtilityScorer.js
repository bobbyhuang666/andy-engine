/**
 * UtilityScorer — 行为候选评分器
 *
 * 纯读取逻辑，不修改任何状态。
 * 无随机，无 domain-specific 术语。
 * goal 评分维度独立，不隐藏在 habit 中。
 *
 * 评分维度：
 *   need, emotion, behavior, memory, relationship, habit, goal, location, world, time, constraint, tendency, total
 */
 ...
 * @property {Object} agent - agent 状态快照
 * @property {Object} world - world 状态快照
 * @property {Object} behaviorField - BehaviorField 快照
 * @property {Object} needs - needs 快照
 * @property {Object} emotion - emotion 快照
 * @property {Object[]} memories - memories 快照
 * @property {Object[]} relationships - relationships 快照
 * @property {Object} goals - goals 快照
 * @property {Object} worldPressure - world pressure 快照
 ...
 * @param {ScoringContext} context - 评分上下文
 ...
 * @param {ScoringContext} context - 评分上下文
 * @returns {Object[]} 每个候选的 { candidate, score }
 */
...
function scoreEmotion(candidate, context) {
  ...
  // 简单规则：负面情绪倾向 rest，正面倾向 explore/socialize
...
/**
 * 行为场维度评分
 * 基于 BehaviorField 当前状态与候选类型的一致性
 */
 ...
 * 记忆维度评分
 *
 * 基于 context.memories 快照的确定性评分。
 * 不访问 live PersonalMemory，不修改 context。
 *
 * 匹配规则（任一命中即算相关）：
 *   - memory.actionType === candidate.type
 *   - memory.target === candidate.target
 *   - memory.associations includes candidate.type or candidate.target
 *   - memory.tags includes candidate.type or candidate.target
 *   - memory.semanticCategory === candidate.metadata?.semanticCategory
 *
 * 分数受 importance/activation 加权，valence 决定方向。
 * Clamped to [-0.5, 0.5].
 */
function scoreMemory(candidate, context) {
  ...
  for (const mem of context.memories) {
    ...
    // 正/中性 valence → boost，负 valence → penalize
...
/**
 * Candidate type → approximate 4D direction vector.
 * Used for tendency alignment scoring.
...
/**
 * 目标维度评分（独立维度，不隐藏在 habit 中）
 *
 * 只读取 context.goals，不访问 Agent 实例。
 * 匹配规则：goal.actionType === candidate.type、goal.target === candidate.target、
 ...
 * 未来倾向维度评分
 *
 * 读取 context.futureTendency 和 context.agent.position。
 * 计算候选类型与当前区域倾向的对齐度。
 * Clamped to [-0.3, 0.3].
 */
function scoreTendency(candidate, context) {
  ...
  // Map candidate type to approximate 4D direction
  const typeVector = CANDIDATE_TYPE_VECTORS[candidate.type];
  if (!typeVector) return 0;

  // Dot product: alignment between tendency and candidate direction
  let alignment = 0;
  for (let d = 0; d < 4; d++) {
 ...
 *
 * 考虑：
 * 1. 候选目标是否是当前区域（已在原地 → 低分）
 * 2. 地点意义对齐（LocationMeaningInfluence）
 */
 ...
 * 世界压力维度评分
 *
 * 读取 context.worldPressure，不修改 context。
 * rest/observe/reflect: 正压力 → 正分（倾向安静行为）
 * work/explore/move: 正压力 → 负分（抑制高活动行为）
 */
function scoreWorld(candidate, context) {
  if (!context.worldPressure) return 0;

  const pressure = context.worldPressure.total || 0;
