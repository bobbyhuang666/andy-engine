...
class FactEmitter {
   ...
   *
   * The observationAction section is intentionally owned by the domain. It
   * supplies both the exact generic interaction strings to recognize and the
   * templates used to render a mapped state. Incomplete domain configuration
   * falls back to the original interaction content.
   *
   * **BOUNDARY**: FactEmitter 是只读 fact 生成器，不修改 engine 状态。
   * 仅读取 agent.stateMachine.currentState 描述被观察者的当前活动。
   *
   * @private
  ...
  _resolveObservationAction(event, targetId, agents, domain) {
    ...
    const observationAction = domain?.narrativeTemplates?.observationAction;
    const genericTemplates = observationAction?.genericTemplates;

    // The domain owns generic-content identification. Core must not infer it
    // from language-specific text or from string shape.
    if (!Array.isArray(genericTemplates) || !genericTemplates.includes(content)) {
      return content;
    }
